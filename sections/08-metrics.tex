...
alert or undecidable evidence verdict, $t_{\mathrm{complete}}$ the first report equal to the
protocol's declared complete class-set target, and $t_{\mathrm{evidence}}$ completion of the
atomic bundle supporting that report. For a declared episode cut $\theta^*$,
Stable-VCL measures finality of the bundle explicitly labeled historical
as-of-$\theta^*$, not currentness at $t_{\mathrm{stable}}$. Define
\[
\begin{aligned}
t_{\mathrm{stable}}=\inf\{T\geq t_{\mathrm{complete}}:{}&
\mathcal V^{\mathrm{historical}}_{u,\theta^*;T}\text{ is sealed},\\
&\bigwedge_j\mathrm{AdmCut}_j(u,\theta^*;T)\}.
\end{aligned}
\]
Because every required source watermark has passed $\theta^*$, no later
record admissible to that cut can revise the sealed vector; later cuts are new
evaluations, not revisions of this one. We report actuation latency
