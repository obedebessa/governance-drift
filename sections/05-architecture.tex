...
minimal additional evidence streams, joined against the approved snapshot.
...
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{\input{figs/fig-architecture}}
\caption{Tiered governance-drift detection. The approved-state snapshot
(left) is recorded at admission. Evidence tiers (center) cumulatively widen
...
distances of \cref{sec:model-distance} continuously and emits classed drift
verdicts with the remediation family each class implies. Annotations show
which of Scenarios S1--S8 each tier first detects, as measured in
\cref{sec:study}.}
\label{fig:architecture}
\end{figure}

\subsection{Components}
\textbf{Approved-state recorder.} At admission, snapshot
$\Gapp(t_0)$---manifest content, \emph{resolved} digests, policy-version pin,
authorization records with windows, intent linkage, environment
...
($\devd$), which the evaluator must report as such rather than as
consistency.
...
\emph{T2}: authorization records with validity semantics---expiry, scope,
revocation---so that exceptions and approvals are objects with clocks, not
...
\emph{recorded} assumptions $\sigma_0$ in $\Gapp$, from cloud inventories,
...
artifact policy (part of T3)~\cite{binauthz}. What no deployed mechanism
provides, to our knowledge, is the join: all tiers evaluated against the
\emph{admitted basis} $\Gapp$, with validity clocks on the authorization
