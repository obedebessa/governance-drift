...
The distinction is not terminological. Configuration drift is a
\emph{two-place, memoryless} relation
...
tools~\cite{awsconfig,driftctl} implement. Governance drift is a
...
approved-state snapshots, observed state $\Gobs(t)$, time-indexed governance contexts
$\Pol(t), \Auth(t), \Int(t)$, and a component-wise governance-divergence vector
$\GDsub(t)$ plus an observability mask $\ObsMask(t)$, with a reasoned rejection of a primitive scalar distance
(\cref{sec:model}).
...
exact-set and Hamming scoring, an independent composite baseline without the
...
accuracy; 20 balanced benign-change windows produced zero
alarms in 543 polls (\cref{sec:lab}). A secondary S10--S12 extension reached
the exact vector in 45/45 cadence observations; six 300-s benign windows
added 4,680 polls without substantive or epistemic alarms. The execution
establishes bounded realizability and within-laboratory behavior, not natural occurrence,
