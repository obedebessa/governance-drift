...
The same section specifies a pending bounded cross-stack rerun on Argo CD and
Gatekeeper (\cref{sec:lab}); it is designed to exercise the native S1
configuration and S3 policy paths, while S4 retains the shared artifact
adapter. Its reportable outputs will be projected singleton classifications
over those declared evaluated components, not complete formal vectors; frozen
counts remain \texttt{CROSS\_TBD}. The process-isolated audit requires exact output
on two consecutive polls; it does not claim watermark-qualified persistence.
