...
and the executed detectability study (\cref{sec:study}); RQ3 by the study's
latency measurements in event time and by the laboratory protocol
(\cref{sec:lab}) that measures it in wall-clock time on real clusters.
...
\item \textbf{Laboratory protocol} (methodological, not executed): a fully
specified Kubernetes/GitOps laboratory---cluster build, scenario scripts,
measurement procedures for wall-clock time-to-detection and
time-to-evidence---for field execution (\cref{sec:lab}).
\end{enumerate}

Established results are cited; analytic claims are proved in the model;
empirical claims are scoped to the executed closed-world study; the laboratory
and its hypotheses are labeled as designed, not run.
