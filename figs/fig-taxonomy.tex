...
\begin{tikzpicture}[node distance=2.8mm and 3mm]
  ...
  \node[brok, below=2.5mm of pol, minimum width=27mm] (auth)
    {\textbf{authorization drift}\\{\smlbl validity of $A_0$ at $t$}\\{\smlbl tier T2/T3 \; (S2, S4, S8)}};
  ...
  \node[brok, below=2.5mm of evd, minimum width=27mm] (env)
    {\textbf{environment drift}\\{\smlbl $\varepsilon_t$ vs.\ assumptions $\sigma_0$}\\{\smlbl tier T4 \; (S5, S7)}};

  \node[evid, right=8mm of auth.east, minimum width=20mm, yshift=-6mm] (gov)
    {\textbf{governance drift}\\{\smlbl aggregate (Def.~8)}\\{\smlbl vector $\GD(t)$}};

  \draw[flow] (conf.east) to[out=0,in=180] node[lbl, above, pos=0.35]{S1: also governance drift} ($(pol.west)+(0,-2mm)$);
  \foreach \n in {pol,auth,int,evd,env}
    \draw[eflow] (\n.east) to[out=0,in=180] (gov.west);
  \draw[eflow] (conf.south) to[out=-60,in=200] node[lbl, below]{approved-property subset} (gov.south west);
\end{tikzpicture}
