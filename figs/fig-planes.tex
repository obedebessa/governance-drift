...
\begin{tikzpicture}[node distance=3.2mm and 4mm]
  ...
  \node[human, minimum width=24mm] (appr) at (0,0)
    {approval + activation\\history\\{\smlbl $t_i^{\rm app}\leq t_i^{\rm act}$}};
  \node[attn, below=2.5mm of appr, minimum width=20mm] (snap)
    {activated $G^{(i)}\to B_x(t)$\\{\smlbl $m_B,\delta_B,\dig(\pi_B),A_B^{\rm proof},\iota_B,\sigma_B$}};
  ...
  \draw[eflow] (appr.east) -- node[lbl, above, pos=0.52]{activation} (r1.west);
