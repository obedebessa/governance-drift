...
> treats these live outputs as provisional class-set classifications under a
> sequential-snapshot laboratory assumption; separate temporal-cut tests
> validate the formal contract without retroactively making the live reads
> admissible cuts. It scopes this as bounded realizability and
> within-laboratory behavior, not
...
The revised manuscript and package include the definition-faithful closed-world study and an executed repeated live laboratory. The latter covers 180 positive injections observed at three cadences and 20 benign windows, preserves all timeout outcomes, reports S6's expected class set as a provisional classification, and separates actuation, DDL, end-to-end latency, and TTE. Separate temporal-cut tests validate the formal contract; the sequential live observations are not relabeled as complete formal vectors. The remaining empirical boundary is explicit: no natural repository or production estate has yet been measured. The paper therefore claims bounded realizability and within-laboratory behavior, not natural occurrence, field prevalence, or production effectiveness.
