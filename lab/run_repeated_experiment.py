...
SCENARIOS = [
    ...
    ("S6", "unapproved Git rollback", ("intent", "authorization"), "T2"),
    ...
    *, repeat: int, scenario: str, tier: str, expected: tuple[str, ...], timing: dict
    ...
    for cadence in CADENCES:
        ...
        observed = item["observed"] if item else {
            "verdict": "timeout", "class": None, "detail": "no verdict"
        }
        detected = item["detected_mono"] if item else deadline
        evidence_at = item["evidence_mono"] if item else deadline
        ...
        rows.append({
            ...
            "classification_correct": observed_class in expected,
...
def apply_control(control: str) -> None:
    if control == "C1":
        ...
        update_approval(revision=revision)
        base.trigger_flux()
        base.wait_rollout()
    elif control == "C3":
        env = dict(base.os.environ)
...
def write_outputs(out: Path, positives: list[dict], controls: list[dict], args: argparse.Namespace) -> None:
    ...
    platform["design"] = (
        "20 repetitions per scenario with randomized polling phase; "
        "20 balanced benign-control windows"
    )
    payload = {
        "seed": SEED,
    ...
    scenario_lines = [
        ...
        r"\toprule",
        r"ID & Expected set & First verdict & DR & CA & Act. & DDL & P95 & E2E & TTE \\",
        r"\midrule",
    ]
    scenario_summary = []
    for scenario, _, expected, _ in SCENARIOS:
        ...
        ca = sum(as_bool(r["classification_correct"]) for r in detected_rows) / len(detected_rows)
        ...
        scenario_lines.append(
            f"{scenario} & {tex_set(item['expected_class_set'])} & {observed} & {pct(dr)} & {pct(ca)} & "
    ...
    cadence_lines = [
        ...
        r"\toprule",
        r"Cadence & Runs & DR (\%) & CA (\%) & Median DDL & P95 DDL \\",
        r"\midrule",
    ]
    cadence_summary = []
    for cadence in CADENCES:
        ...
        cadence_lines.append(
            f"{cadence:.1f} & {len(rows)} & {pct(dr)} & {pct(ca)} & "
    ...
    control_lines = [
        ...
        r"\toprule",
        r"ID & Benign control & Windows & Polls & Alarm windows \\",
        r"\midrule",
    ]
    control_summary = []
    for control, name in CONTROLS:
        ...
        control_lines.append(
            f"{control} & {name} & {windows} & {polls} & {alarm_windows} \\\\"
        )
...
def main() -> None:
    ...
    for repeat, (scenario, injection, expected, tier) in tasks:
        ...
        print(
            ...
            + ", ".join(
                f"{r['cadence_seconds']}s:{r['observed_class']}/DDL={float(r['ddl_seconds']):.2f}s"
                for r in rows
            ),
            flush=True,
        )
