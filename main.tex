...
% ------------------------- abstract ----------------------------------
\noindent\textbf{\textit{Abstract}---Configuration drift compares desired and observed configuration; it cannot decide whether a running system remains covered by what authorized it. We define \emph{governance drift} as a history-anchored relation among operational reality, current governance context, and a unique activated approval basis. The model separates five substantive components from epistemic observability and specifies stable unit identity, activation and supersession, and admissible temporal cuts. In a twelve-scenario closed world, a cumulative ablation raises unconditional exact-vector success from 75.0\% for plane-local signals to 100\% for the admitted-basis join; ten executable probes isolate the incremental semantics. On a version-pinned Kind/Flux/Kyverno stack, 180 injections observed at three cadences yielded 538/540 detections and 538/538 exact provisional class-set classifications conditional on detection under the laboratory's sequential-snapshot assumption; a scenario-stratified cluster bootstrap gives a 95\% interval of 98.89--100\% for detection over the balanced experimental mixture. During 20 benign transitions, 777 polls produced no policy, authorization, intent, or environment drift; seven configuration-convergence and 36 fail-safe evidence-warning polls were retained, and all 60 window--cadence trajectories ended provisionally consistent. A pending separate-stack Argo CD/Gatekeeper rerun is limited to projected singleton classifications over declared evaluated components: configuration and policy use native paths, whereas authorization reuses the shared digest adapter; its frozen counts remain \texttt{CROSS\_TBD}. A separate nine-episode audit used 27 isolated observer-process trajectories; all 27 reached the exact expected class set and then two-poll exact persistence across 261 polls with zero adapter error. These bounded results demonstrate semantic realizability and safe uncertainty handling within the tested conditions, not natural prevalence or production effectiveness.}
...
\ifanonymoussubmission
  \input{sections/10a-availability-anonymous}
\else
